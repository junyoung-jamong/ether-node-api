...
router.get('/disk/:node_id', function(req, res){
    ...
    async.parallel([
        ...
        function(callback){
            const query = PROM_HOST + "/api/v1/query?query=sum(node_filesystem_avail_bytes{id='" + nodeId + "'})%20by%20(id)/1024/1024/1024";
            getNodeResource(query, callback);
        }
    ], function(err, results){
        if(err){
...
router.get('/network/:node_id', function(req, res){
    ...
    async.parallel({
        ...
        trans: function(callback){
            const query = PROM_HOST + "/api/v1/query_range?query=sum(increase(node_network_transmit_bytes_total{id='" + nodeId + "'}[1m]))/1024/1024&start=" + start + "&end=" + end + "&step=" + interval
            getNodeResourceGraph(query, callback);
        }
    }, function(err, results){
        if(err){
        ...
        if(!results.recv.data || !result.trans.data){
            res.json({
                result: false,
                message: "No data"
            })
