...
router.get('/memory/:node_id', function(req, res){
    ...
    const query = PROM_HOST + "/api/v1/query_range?query=1-(node_memory_MemAvailable_bytes/node_memory_MemTotal_bytes{id='" + nodeId + "'})&start=" + start + "&end=" + end + "&step=" + step
    requestNodeGraphQuery(query, res);
})
...
router.get('/disk/:node_id', function(req, res){
    ...
    const query = PROM_HOST + "/api/v1/query_range?query=1-(sum(node_filesystem_avail_bytes{id='" + nodeId + "'})%20by%20(id)/sum(node_filesystem_size_bytes)%20by%20(id))&start=" + start + "&end=" + end + "&step=" + step
...
router.get('/network_recv/:node_id', function(req, res){
    ...
    const query = PROM_HOST + "/api/v1/query_range?query=sum(increase(node_network_receive_bytes_total{id='" + nodeId + "'}[1m]))/1024/1024&start=" + start + "&end=" + end + "&step=" + step
	return requestNodeRangeQuery(query)
})
...
router.get('/network_trans/:node_id', function(req, res){
    ...
    const query = PROM_HOST + "/api/v1/query_range?query=sum(increase(node_network_transmit_bytes_total{id='" + nodeId + "'}[1m]))/1024/1024&start=" + start + "&end=" + end + "&step=" + step
	return requestNodeRangeQuery(query)
})
