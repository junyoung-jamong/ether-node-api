...
const QUERY_CPU_USAGE_LIST = PROM_HOST + "/api/v1/query?query=1-(avg%20by%20(id)(irate(node_cpu_seconds_total{mode=%27idle%27}[5m])))";
const QUERY_MEMORY_USAGE_LIST = PROM_HOST + "/api/v1/query?query=1-(node_memory_MemAvailable_bytes/node_memory_MemTotal_bytes)";
const QUERY_DISK_USAGE_LIST = PROM_HOST + "/api/v1/query?query=1-(node_filesystem_avail_bytes{device='rootfs'}/node_filesystem_size_bytes{device='rootfs'})";
const QUERY_NETWORK_RECEIVE_LIST = PROM_HOST + "/api/v1/query?query=sum%20by%20(id)(increase(node_network_receive_bytes_total[1m]))/60/1024/1024";
const QUERY_NETWORK_TRANSMIT_LIST = PROM_HOST + "/api/v1/query?query=sum%20by%20(id)(increase(node_network_transmit_bytes_total[1m]))/60/1024/1024";
...
router.get('/cpu/:node_id', function(req, res){
    ...
    async.waterfall([
        function(callback){
            ...
            getNodeResourceGraph(query, callback);
        }
    ], function(err, result){
        if(err){
            res.json({
...
router.get('/memory/:node_id', function(req, res){
    ...
    async.parallel([
        ...
        function(callback){
            const query = PROM_HOST + "/api/v1/query?query=node_memory_MemAvailable_bytes{id='" + nodeId + "'}/1024/1024/1024"
            getNodeResource(query, callback);
        }
    ], function(err, results){
        if(err){
            res.json({
        ...
        result = results[0];
        result.available = results[1].data;
...
router.get('/disk/:node_id', function(req, res){
    ...
    async.parallel([
        function(callback){
            const query = PROM_HOST + "/api/v1/query_range?query=1-(node_filesystem_avail_bytes{device='rootfs',id='" + nodeId + "'}/node_filesystem_size_bytes)&start=" + start + "&end=" + end + "&step=" + interval
            getNodeResourceGraph(query, callback);
        },
        function(callback){
            const query = PROM_HOST + "/api/v1/query?query=sum(node_filesystem_avail_bytes{device='rootfs',id='" + nodeId + "'})%20by%20(id)/1024/1024/1024";
            getNodeResource(query, callback);
        }
    ], function(err, results){
        if(err){
...
router.get('/network/:node_id', function(req, res){
    const nodeId = req.params.node_id;
    ...
    async.parallel({
        recv: function(callback){
            const query = PROM_HOST + "/api/v1/query_range?query=sum(increase(node_network_receive_bytes_total{id='" + nodeId + "'}[1m]))/60/1024/1024&start=" + start + "&end=" + end + "&step=" + interval;
            getNodeResourceGraph(query, callback);
        },
        trans: function(callback){
            const query = PROM_HOST + "/api/v1/query_range?query=sum(increase(node_network_transmit_bytes_total{id='" + nodeId + "'}[1m]))/60/1024/1024&start=" + start + "&end=" + end + "&step=" + interval;
            getNodeResourceGraph(query, callback);
        }
    }, function(err, results){
        if(err){
        ...
        if(!results.recv.data || !results.trans.data){
            res.json({
                result: false,
                message: "No data"
            })
        ...
        var data = {
            network_recv: results.recv.data,
            network_trans: results.trans.data,
        };

        var count = results.recv.data.length;
...
function getNodeResourceGraph(query, callback){
    request(query, function(err, response, body){
        if(err){
            var result = {
                result: false,
...
function getNodeResource(query, callback){
    request(query, function(err, response, body){
        if(err){
            var result = {
                result: false,
        ...
        var data = queryResults[0].value[1];
